...
/* Answer : This query calculates the total revenue earned by each store.  It links the `payment` table to the `rental`, `inventory`, and `store` tables to track 
every payment back to the linked store.  The SUM(payment.amount) function determines the overall revenue by totalling up all 
payments made for rentals.  The GROUP BY (store.store_id) term ensures that the revenue is collected for each store independently.  
The result provides a clear representation of the total money earned by each store, with columns representing the store ID and its matching total revenue. */
